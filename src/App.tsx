...
export function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      {/* Top MNC Companies Hiring with Opening buttons */}
      <TopCompanies
      ...
      {/* What Clyptus Does & About Clyptus */}
      <AboutClyptus 
        onLogin={() => triggerLoginPrompt()}
      />

      {/* Footer with Directory & Clyptus Attribution */}
      <Footer
      ...
      {toastMessage && (
        <div style={{
          ...
          <Lock size={18} color="#FF6600" />
          <span>{toastMessage}</span>
        </div>
      )}

      {/* Simple Please Login Modal Dialog */}
      {loginAlertOpen && (
        <div 
          className="modal-backdrop"
          onClick={(e) => {
            if (e.target === e.currentTarget) setLoginAlertOpen(false);
          }}
          style={{
            position: 'fixed',
            inset: 0,
